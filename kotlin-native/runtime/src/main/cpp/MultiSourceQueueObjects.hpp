...
class MultiSourceQueueObjects {
public:
    class Producer;

    // TODO: Consider switching from `KStdListObjects` to `SingleLockList` to hide the constructor
    ...
    private:
        friend class MultiSourceQueueObjects;

        T value_;
        std::atomic<Producer*> owner_; // `nullptr` signifies that `MultiSourceQueueObjects` owns it.
        typename KStdListObjects<Node>::iterator position_;
    };

    class Producer {
    public:
        explicit Producer(MultiSourceQueueObjects& owner) noexcept : owner_(owner) {}
        ...
        // Merge `this` queue with owning `MultiSourceQueueObjects`. `this` will have empty queue after the call.
    ...
    private:
        MultiSourceQueueObjects& owner_; // weak
        KStdListObjects<Node> queue_;
        KStdListObjects<Node*> deletionQueue_;
    };
    ...
    private:
        ...
        explicit Iterator(const typename KStdListObjects<Node>::iterator& position) noexcept : position_(position) {}
    ...
    private:
        ...
        explicit Iterable(MultiSourceQueueObjects& owner) noexcept : owner_(owner), guard_(owner_.mutex_) {}

        MultiSourceQueueObjects& owner_; // weak
        std::unique_lock<Mutex> guard_;
    };

    // Lock `MultiSourceQueueObjects` for safe iteration. If element was scheduled for deletion,
    // it'll still be iterated. Use `ApplyDeletions` to remove those elements.
    Iterable LockForIter() noexcept { return Iterable(*this); }

    // Lock `MultiSourceQueueObjects` and apply deletions. Only deletes elements that were published.
    void ApplyDeletions() noexcept {
        std::lock_guard<Mutex> guard(mutex_);
        KStdListObjects<Node*> remainingDeletions;
...
private:
    // Using `KStdListObjects` as it allows to implement `Collect` without memory allocations,
